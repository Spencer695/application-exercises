...
**Answer:** There are 200 countries in the data set.
...
The human rights for the united kingdom back in 1960 was around 30% and it dipped to 25% around 1980. Latter at 2000 it jumps to 50% then starts to decrease to about 40% in 2020. 

**Task 2.2:** Which country appears most consistent in its voting patterns across all issues? Which appears least consistent?

**Your answer:**

The country that appears the most consistent is the united kingdom across all issues, while the united state appears the least consistent across all issues. 
...
The most similar is nuclear weapons and nuclear material, because all three lines are close together on the chart.

Most different:

The most different is Palestinian conflict, because all three are on different parts of the graph as speperate as possible. 
...
unvotes %>%
  filter(country %in% c("Canada", "Japan", "Germany")) %>%  # Change these!
  mutate(year = year(date)) %>%
  group_by(country, year, issue) %>%
  summarize(percent_yes = mean(vote == "yes")) %>%
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) +
...
**Which countries did you choose?** Canada, Japan, and Germany

**Task 3.2:** Describe one interesting pattern you notice in your new plot.

**Your answer:**

These three countries are very similar for almost all of the voting patterns. Also Germany doesn't show up in the voting until about 1990. 
...
unvotes %>%
  filter(country %in% c("Canada", "Japan", "Germany", "China", "Russia")) %>%  # Choose 5 countries
  ...
  scale_y_continuous(labels = percent) +
  labs(
    title = "Percentage of 'Yes' votes in the UN General Assembly",  # Update title
    subtitle = "1946 to 2019",
    y = "% Yes",
    x = "Year",
    color = "Country"
...
For the five countries the lowest yes votes for human rights in 2020 was Canada, while the highest was China. In 1980 Canada and Japan were about 30% while China and Russia was about 85% yes votes. 
...
**Which country has the highest overall percentage of "yes" votes?** 

Seychelles is the overall highest percentage of yes votes.

**Which has the lowest?** 

Maldives is the overall lowest percentage of yes votes. 
...
**Which issue has the most "yes" votes on average?** 

Palestinian conflict has the most yes votes on average.

**Which has the least?** 

Human rights has the least yes votes on average. 
